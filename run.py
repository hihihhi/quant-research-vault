...
def run_step(script: str, extra_args: list[str]) -> int:
    ...
    try:
        ...
        print(
            f"\n[run.py] {script} exceeded {timeout // 60} min timeout — killing.",
            flush=True,
        )
...
def main() -> None:
    ...
    if args.workers is None and not args.abstract_only:
        try:
            ...
            print(
                f"[auto] Using {args.workers} Claude workers "
                f"(available RAM ~{ram_gb:.1f}GB, CPU cores/2={cpu})",
                flush=True,
            )
        except ImportError:
            args.workers = 2
            print(
                "[auto] psutil not found; defaulting to 2 Claude workers.", flush=True
            )
    ...
    if args.all_history:
        ...
        print(
            f"All-history pipeline: {len(windows)} quarterly windows ({start_date} to {end_date})"
        )
        ...
        for i, (w_start, w_end) in enumerate(windows, 1):
            ...
            if rc != 0:
                print(
                    f"Fetch failed for chunk {i}. Waiting 120s then continuing...",
                    flush=True,
                )
                _time.sleep(120)
                continue
            ...
            total, pending = count_db(cfg["db_path"])
            print(
                f"\n[Chunk {i}/{len(windows)} done] DB: {total} total, {pending} still pending",
                flush=True,
            )
